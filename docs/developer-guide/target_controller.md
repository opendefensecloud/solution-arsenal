...
| --------------- | --------------------------------------------------------- |
| Release         | `render-rel-<release-name>-<hash>` (truncated to 253 chars) |
| Bootstrap       | `render-tgt-<target-name>-<bootstrapVersion>`             |

The release RenderTask name includes a hash derived from the release name, target name, and release generation to ensure uniqueness across namespace-scoped targets.

## Bootstrap Versioning

`target.status.bootstrapVersion` is a monotonically increasing counter. It is incremented whenever the desired bootstrap input changes — i.e. when the set of bound releases (or their resolved chart URLs / userdata) changes. This guarantees a new chart version is pushed to the OCI registry whenever content changes.

Stale bootstrap and release RenderTasks (from prior versions) are deleted once the current bootstrap RenderTask completes successfully.
...
### Initial deployment: Target with one Release
...
sequenceDiagram
    ...
    participant Registry as zot-deploy (OCI)

    User->>K8s: Create Registry "deploy-registry"<br/>(hostname, solarSecretRef)
    User->>K8s: Create Target "cluster-1"<br/>(renderRegistryRef: deploy-registry)
    User->>K8s: Create ReleaseBinding (cluster-1 → ocm-demo-release)
    K8s->>TargetCtrl: Reconcile(Target/cluster-1)

    TargetCtrl->>K8s: Get Registry "deploy-registry"
    K8s-->>TargetCtrl: Registry resolved
    TargetCtrl->>K8s: Update status (RegistryResolved=True)

    TargetCtrl->>K8s: List ReleaseBindings for cluster-1
    K8s-->>TargetCtrl: [ReleaseBinding → ocm-demo-release]
    TargetCtrl->>K8s: Get Release → ComponentVersion

    TargetCtrl->>K8s: Create RenderTask "render-rel-ocm-demo-release-<hash>"
    K8s->>RenderTaskCtrl: Reconcile(RenderTask render-rel-*)
    RenderTaskCtrl->>Registry: Push release chart<br/><testns>/release-ocm-demo-release:v0.0.1
    RenderTaskCtrl->>K8s: Update RenderTask status (JobSucceeded=True, ChartURL=...)

    K8s->>TargetCtrl: Reconcile(Target) [RenderTask status changed]
    TargetCtrl->>K8s: Update status (ReleasesRendered=True)
    TargetCtrl->>K8s: Create RenderTask "render-tgt-cluster-1-0" (bootstrapVersion=0)
    K8s->>RenderTaskCtrl: Reconcile(RenderTask render-tgt-cluster-1-0)
    RenderTaskCtrl->>Registry: Push bootstrap chart<br/><testns>/bootstrap-cluster-1:v0.0.0
    RenderTaskCtrl->>K8s: Update RenderTask status (JobSucceeded=True, ChartURL=...)
...
sequenceDiagram
    ...
    participant RenderTaskCtrl as RenderTask Controller
    participant Registry as zot-deploy (OCI)
    participant FluxCD as FluxCD (target cluster)

    User->>K8s: Create Profile<br/>(targetSelector: matches cluster-1, releaseRef: profile-ocm-demo-release)
    K8s->>ProfileCtrl: Reconcile(Profile)
    ProfileCtrl->>K8s: Create ReleaseBinding (cluster-1 → profile-ocm-demo-release)

    K8s->>TargetCtrl: Reconcile(Target/cluster-1) [new ReleaseBinding]
    TargetCtrl->>K8s: Create RenderTask "render-rel-profile-ocm-demo-release-<hash>"
    K8s->>RenderTaskCtrl: Reconcile(RenderTask render-rel-*)
    RenderTaskCtrl->>Registry: Push release chart<br/><testns>/release-profile-ocm-demo-release:v0.0.1
    RenderTaskCtrl->>K8s: Update RenderTask status (JobSucceeded=True)

    K8s->>TargetCtrl: Reconcile(Target) [RenderTask status changed]
    TargetCtrl->>K8s: Update status (ReleasesRendered=True)
    Note over TargetCtrl: Bootstrap input changed (2nd release added)<br/>→ increment bootstrapVersion: 0 → 1
    TargetCtrl->>K8s: Create RenderTask "render-tgt-cluster-1-1" (bootstrapVersion=1)
    K8s->>RenderTaskCtrl: Reconcile(RenderTask render-tgt-cluster-1-1)
    RenderTaskCtrl->>Registry: Push bootstrap chart<br/><testns>/bootstrap-cluster-1:v0.0.1
    RenderTaskCtrl->>K8s: Update RenderTask status (JobSucceeded=True)

    K8s->>TargetCtrl: Reconcile(Target) [RenderTask status changed]
    TargetCtrl->>K8s: Update status (BootstrapReady=True, bootstrapVersion=1)
    TargetCtrl->>K8s: Delete stale RenderTask "render-tgt-cluster-1-0"

    Note over FluxCD: FluxCD polls OCIRepository "solar-bootstrap"
    FluxCD->>Registry: Detect new artifact v0.0.1
    FluxCD->>FluxCD: Upgrade HelmRelease "solar-bootstrap"
    FluxCD->>FluxCD: Bootstrap chart creates 2 inner HelmReleases<br/>(solar-bootstrap-ocm-demo-release + solar-bootstrap-profile-ocm-demo-release)
    FluxCD->>FluxCD: Inner HelmReleases install workload Deployments
```
