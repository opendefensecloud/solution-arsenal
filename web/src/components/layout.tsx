    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className="flex w-60 flex-col border-r border-sidebar-border bg-sidebar">
        ...
        <div className="flex h-14 items-center gap-2.5 border-b border-sidebar-border px-4">
          ...
          <div>
            <span className="text-lg font-bold tracking-tight text-primary">SolAr</span>
            <p className="text-[12px] leading-none text-muted-foreground">Solution Arsenal</p>
          </div>
        </div>
              ...
              {isImpersonating ? (
                <>
                  <div className="space-y-0.5 text-xs">
                    <p className="font-medium text-foreground">{impersonatedUsername}</p>
          ...
          {user?.authenticated && (
            <div className="flex items-center justify-between rounded-md bg-accent/50 px-3 py-2">
              <div className="min-w-0">
                ...
                {user.groups?.[0] && (
                  <p className="truncate text-xs text-muted-foreground">{user.groups.join(', ')}</p>
                )}
              </div>
              <a
