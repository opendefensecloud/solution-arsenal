...
export function TargetsPage() {
  ...
  const { data: bindingsData } = useQuery(releaseBindingQueries.list(namespace));
  ...
  const filtered = useMemo(() => {
    ...
    if (namespaceFilter.size > 0) {
      result = result.filter((t) => namespaceFilter.has(t.metadata.namespace));
    }
  ...
  }, [allTargets, ls.search, ls.sortField, ls.sortDir, namespaceFilter]);
                    ...
                    {ls.tileView ? (
                      <div className="flex flex-col h-full">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm font-semibold text-foreground truncate flex-1">{target.metadata.name}</h3>
                        </div>
                        <p className="mt-1.5 text-xs text-muted-foreground font-mono truncate">{target.spec.renderRegistryRef.name}</p>
                        <p className="text-xs text-muted-foreground flex-1">{target.metadata.namespace}</p>
                        <div className="mt-2 flex items-center justify-between">
                          <span className="text-xs text-muted-foreground">{bindingCount} release{bindingCount !== 1 ? "s" : ""} bound</span>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h3 className="text-base font-semibold text-foreground">{target.metadata.name}</h3>
                            <p className="text-sm text-muted-foreground">
                              {target.metadata.namespace} &middot; {target.spec.renderRegistryRef.name} &middot; {bindingCount} release{bindingCount !== 1 ? "s" : ""} bound
                            </p>
                          </div>
                        </div>
                        <StatusBadge conditions={target.status?.conditions} />
