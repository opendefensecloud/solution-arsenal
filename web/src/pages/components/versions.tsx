...
export function ComponentVersionsPage() {
  ...
  const { data: comp, isLoading, isError, error } = useQuery(componentQueries.detail(namespace, name));
  const { data: versionsData, isError: versionsError } = useQuery(componentVersionQueries.list(namespace));

  const [search, setSearch] = useState("");

  const versions = useMemo(() => {
    const list = (versionsData?.items ?? []).filter(
      (v) => v.spec.componentRef.name === name,
    );
    return list.sort((a, b) =>
      b.spec.tag.localeCompare(a.spec.tag, undefined, { numeric: true }),
    );
  }, [versionsData, name]);
  ...
  const filtered = useMemo(() => {
    ...
    return versions.filter(
      (cv) =>
        cv.spec.tag.toLowerCase().includes(q) ||
        primaryRepository(cv).toLowerCase().includes(q),
    );
  }, [versions, search]);
          ...
          {filtered.map((cv) => (
            <div key={cv.metadata.name} className="flex items-center gap-3 px-4 py-3">
              <div className="flex shrink-0 items-center justify-center rounded-md bg-muted px-2 py-1">
                <span className="text-xs font-mono font-semibold text-foreground">{cv.spec.tag}</span>
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-mono text-foreground truncate">{primaryRepository(cv)}</p>
              </div>
              {comp?.spec.registry && (
